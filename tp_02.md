...
```sql
-- suppression de la colonne plane de la table pilots
ALTER TABLE `pilots` DROP `plane`;
```

```sql
-- Création de la colonne plane_id 
...
réponse : un pilote peut avoir plusieurs vol et un vol peut avoir qu'un seul pilote
...
## 06 Exercice sauvegarde de la base de données
